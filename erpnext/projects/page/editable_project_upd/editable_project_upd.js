frappe.pages['editable_project_upd'].on_page_load = function (wrapper) {
    ...
    function show_add_new_dialog() {
        const dialog = new frappe.ui.Dialog({
            ...
            fields: [
                ...
                { label: 'Date', fieldname: 'date', fieldtype: 'Date', reqd: 1, default: frappe.datetime.nowdate() },
            ...
            primary_action(values) {
                frappe.call({
                    ...
                    callback: function(r) {
                        if (!r.exc) {
                            frappe.show_alert({ message: __('New update created successfully'), indicator: 'green' });
                            dialog.hide();
                            load_data();
                        }
                    }
    ...
    function show_edit_dialog(row_data) {
        const dialog = new frappe.ui.Dialog({
            ...
            fields: [
                ...
                { label: 'Done Task', fieldname: 'done_task', fieldtype: 'Text', default: row_data.done_task },
                { label: 'Resources', fieldname: 'recources', fieldtype: 'Text', default: row_data.recources },
                { label: 'Risks / Red Flags', fieldname: 'risks_red_flags', fieldtype: 'Text', default: row_data.risks_red_flags },
                { label: 'Decision / Support', fieldname: 'decision_support', fieldtype: 'Text', default: row_data.decision_support },
                { label: 'Previous Issues', fieldname: 'previous_issues', fieldtype: 'Text', default: row_data.previous_issues },
                { label: 'Deadline', fieldname: 'deadline', fieldtype: 'Date', default: row_data.deadline },
                { label: 'Deadline Description', fieldname: 'deadline_desciption', fieldtype: 'Text', default: row_data.deadline_desciption },
                { label: 'Next Milestones', fieldname: 'next_milestones', fieldtype: 'Text', default: row_data.next_milestones }
            ],
            primary_action_label: __('Save'),
            primary_action(values) {
                ...
                frappe.call({
                    ...
                    callback: function(r) {
                        if (r.message && r.message.status === "success") {
                            frappe.show_alert({ message: __('Update saved successfully'), indicator: 'green' });
                            dialog.hide();
                            load_data();
                        } else {
    ...
    function render_table() {
        ...
        const columns = [
            ...
            { id: 'project_name', label: 'Project', format: (v, row) => `<a href="/app/project/${row.parent}" target="_blank">${v}</a>` },
    ...
    table_container.on('click', '.btn-delete', function() {
        const row_name = $(this).attr('data-row-name');
        frappe.confirm(
            __('Are you sure you want to delete this project update?'),
            () => {
                frappe.call({
